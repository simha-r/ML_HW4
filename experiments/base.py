...
class EvaluationStats(object):
    def __init__(self):
        ...
        self.reward_min = 0
        self.runs = 0
        self.num_of_steps_list = list()
        self.num_of_steps = 0

	# add reward to reward list
    def add(self, reward, num_of_steps):
        self.rewards.append(reward)
        self.num_of_steps_list.append(num_of_steps)

        # self.compute()

	# compute reward stats and save it stat_history
    def compute(self):
        ...
        self.num_of_steps = np.mean(self.num_of_steps_list)
...
class BaseExperiment(ABC):
    ...
    def run_solver_and_collect(self, solver, convergence_check_fn):
        ...
        stats.total_iteration_step = step_count
        return stats

    def run_policy_and_collect(self, solver, discount, policy,  times=100):
        stats = EvaluationStats()
        reward_array = []
        for i in range(times):
            # stats.add(np.sum(solver.run_policy(policy)))
            p = solver.run_policy(policy)
            stats.add(np.sum(p),len(p))
            reward_array.append(np.sum(p))
            # print(solver.run_policy(policy))
        # print("Reward array --- ", discount, np.mean(reward_array))
        stats.compute()
